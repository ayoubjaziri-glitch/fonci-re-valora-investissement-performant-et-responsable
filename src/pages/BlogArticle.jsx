...
import { 
  ...
  Facebook, Linkedin, Mail, ChevronRight, Link2, Check
} from 'lucide-react';
...
function ShareButtons({ handleShare, copied }) {
  return (
    <div className="flex items-center gap-2 flex-wrap mb-12 pb-8 border-b border-slate-200">
      <span className="text-sm text-slate-600 font-medium mr-1">Partager :</span>
      <button onClick={() => handleShare('linkedin')} title="LinkedIn"
      ...
      <button onClick={() => handleShare('twitter')} title="X / Twitter"
        className="w-10 h-10 rounded-full bg-slate-100 hover:bg-black hover:text-white flex items-center justify-center transition-colors">
        <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-4.714-6.231-5.401 6.231H2.746l7.73-8.835L1.254 2.25H8.08l4.259 5.63 5.905-5.63zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>
      </button>
      <button onClick={() => handleShare('whatsapp')} title="WhatsApp"
        className="w-10 h-10 rounded-full bg-slate-100 hover:bg-[#25D366] hover:text-white flex items-center justify-center transition-colors">
      ...
      <button onClick={() => handleShare('email')} title="Email"
      ...
      <button onClick={() => handleShare('copy')} title="Copier le lien"
        className={`w-10 h-10 rounded-full flex items-center justify-center transition-colors ${copied ? 'bg-emerald-500 text-white' : 'bg-slate-100 hover:bg-slate-300'}`}>
...
- **Économie : 6 400 €**`
  ...
  // Fusionner BDD + statiques (priorité BDD), priorité à la BDD
  const allArticles = [
    ...dbArticles,
  ...
  const handleShare = (platform) => {
    ...
    const shareUrls = {
      ...
      email: `mailto:?subject=${encodeURIComponent(text)}&body=${encodeURIComponent('Je vous partage cet article : ' + url)}`
    };
    ...
    <div className="min-h-screen bg-white">
      ...
      <article className="max-w-4xl mx-auto px-6 lg:px-8 py-12">
        ...
        <motion.div
          ...
          {/* Share Buttons - top */}
          <ShareButtons handleShare={handleShare} copied={copied} />
        </motion.div>
        ...
        <div className="mt-12 pt-8 border-t border-slate-200">
          <p className="text-center text-slate-500 text-sm mb-4">Vous avez aimé cet article ? Partagez-le !</p>
