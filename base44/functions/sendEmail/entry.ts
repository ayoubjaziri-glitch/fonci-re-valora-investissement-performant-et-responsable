// Backend function pour envoyer les emails de contact
// À déployer sur Ionos ou utiliser avec un proxy
...
export default async function handler(req, res) {
  ...
  try {
    const { prenom, nom, email, telephone, type_demande, message, destinataires } = req.body;
...
<body style="margin:0;padding:0;background:#f4f4f4;font-family:Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#f4f4f4;padding:40px 0;">
    ...
    for (const to of toList) {
      ...
      if (!resendRes.ok) {
        throw new Error(`Resend API error: ${resendRes.statusText}`);
      }
    }
